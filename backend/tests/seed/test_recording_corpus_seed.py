...
@pytest.mark.asyncio
async def test_recording_corpus_seed(engine: AsyncEngine, database: DbAccessor, uow: UnitOfWork) -> None:
    ...
    async with SqlAlchemyMusicCatalogUnitOfWork(session_factory) as music_uow:
        recordings = [
            await music_uow.recordings.get_published(recording_id) for recording_id, _ in data.SEED_RECORDINGS
        ]
        english = await music_uow.lyrics_versions.get_published(data.SIXTEEN_TONS_EN_LYRICS_ID)
        russian = await music_uow.lyrics_versions.get_published(data.SIXTEEN_TONS_RU_READING_ID)
        translation = await music_uow.lyrics_version_relations.get_published(data.SIXTEEN_TONS_TRANSLATION_RELATION_ID)
    ...
    async with SqlAlchemyHistoricalKnowledgeUnitOfWork(session_factory) as historical_uow:
        fragment = await historical_uow.sources.get_fragment(data.SIXTEEN_TONS_FRAGMENT_ID)
        origin = await historical_uow.recording_origin_claims.get(data.MERLE_FIRST_RECORDING_CLAIM_ID)
        guide = await historical_uow.listening_guides.get(data.FORD_LISTENING_GUIDE_ID)
