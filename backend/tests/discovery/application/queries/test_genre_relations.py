...
async def test_relations_query_maps_perspective_sort_and_evidence() -> None:
    ...
    query = GenreRelationsQuery(StubGenreReader(genres=tuple(music.values()), by_id=music), claim_reader)
...
async def test_relations_query_returns_empty_for_published_genre_without_relations() -> None:
    ...
    genres = {swing.id: swing}
    query = GenreRelationsQuery(
        StubGenreReader(genres=tuple(genres.values()), by_id=genres),
        claim_reader,
    )
...
async def test_relations_query_hides_missing_and_non_public_genres(status: GenreEditorialStatus | None) -> None:
    del status
    genre_id = uuid7()
    claim_reader = _claim_reader({}, FakeSourceRepository(), {})
    query = GenreRelationsQuery(StubGenreReader(by_id={}), claim_reader)
...
async def test_relations_query_rejects_claim_unrelated_to_page_genre() -> None:
    ...
    genres = {swing.id: swing, jazz.id: jazz, blues.id: blues}
    query = GenreRelationsQuery(
        StubGenreReader(genres=tuple(genres.values()), by_id=genres),
