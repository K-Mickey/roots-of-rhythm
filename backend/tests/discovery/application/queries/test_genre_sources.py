...
def _query(
    ...
    *,
    ...
    visible_music = music if published is None else {gid: g for gid, g in music.items() if gid in published}
    source_map = dict(sources.sources)
    return GenreSourcesQuery(
        StubGenreReader(genres=tuple(visible_music.values()), by_id=visible_music),
        FakePublishedGenreRelationClaimReader(claims, sources),
        StubSourceReader(source_map),
    )
