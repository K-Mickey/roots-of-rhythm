...
from tests.music_catalog.fakes import FakeGenreRepository, FakeMusicCatalogUnitOfWork
...
@pytest.mark.asyncio
async def test_song_reader_batches_work_and_recording_genres_once() -> None:
    work_id = uuid7()
    recording_id = uuid7()
    ...
    uow = FakeMusicCatalogUnitOfWork(
        {work_genre.id: work_genre, recording_genre.id: recording_genre},
        assignments,
        works={work_id: work},
    ...
    data = await SqlAlchemySongMusicReader(lambda: uow).get_song_data(work_id)
