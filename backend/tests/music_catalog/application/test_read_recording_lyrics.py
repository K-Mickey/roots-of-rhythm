...
@pytest.mark.asyncio
async def test_recording_lyrics_read_service_returns_reading_translations_and_fallback() -> None:
    first_work_id = uuid7()
    work_id = uuid7()
    ...
    lyrics_repo = SpyLyricsVersionRepository(versions)
    scope = fake_transaction_scope()
    service = RecordingLyricsReadService(
        scope,
        lambda _t: lyrics_repo,
        lambda _t: FakeLyricsVersionRelationRepository(relations),
    )
    ...
    assert lyrics_repo.published_for_works_calls == [{first_work_id, work_id}]
