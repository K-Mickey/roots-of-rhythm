...
class SourceService:
    ...
    async def create_source(self, title: str, *, institution_name: str | None = None) -> Source:
        source = Source.create(title, institution_name=institution_name)
        async with self._uow_factory() as uow:
            await uow.sources.add_source(source)
            await uow.commit()
            return source

    async def create_version(self, source_id: UUID, label: str) -> SourceVersion:
        async with self._uow_factory() as uow:
            if await uow.sources.get_source(source_id) is None:
                raise SourceNotFound(str(source_id))
            version = SourceVersion.create(source_id, label)
