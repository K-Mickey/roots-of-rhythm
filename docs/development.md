...
| `make test-e2e` | запущенный `make up` + `make seed`, Chromium | `e2e/preflight.ts` проверяет Chromium и health обоих сервисов и завершает прогон одной подсказкой команд до запуска воркеров; дальше Playwright: основные каталоги и detail pages, Recording catalog/detail, интерактивная Song page Sixteen Tons, not-found, browser history и mobile layout. |
