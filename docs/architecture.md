...
Вторая проекция — `historical_knowledge`: GenreRelation Claim с Evidence references и Source/SourceVersion/SourceFragment aggregates. Source хранит bibliographic metadata (title, author, responsible organization, publication, publication date, canonical external URL); SourceFragment хранит citation locator. Create/publish Claim используют transaction-only boundary и отдельно внедрённые Historical Knowledge/Music Catalog repositories одной PostgreSQL session. Historical Knowledge public reader возвращает опубликованные Claims и reviewed evidence, а Discovery применяет межконтекстную видимость endpoint Genre.
