...
- Минимальные каталоги: исполнители, группы, песни, записи, релизы (+ уже существующий `/genres`).
...
- **EPIC-002-OQ-001 — пользовательские тексты и notice-and-takedown.** Нужно отдельно решить, допускает ли продукт загрузку полного текста пользователем на основании его декларации о правах, какие moderation/review, provenance, audit trail, complaint и takedown-процессы обязательны и в каких юрисдикциях работает сервис. До решения controlled corpus публикует полный body только для public-domain или документированно лицензированных LyricsVersion; для остальных сохраняются metadata, rights-aware причина недоступности и внешние ссылки. Решение требует legal review, отдельной story и ADR; редакционный seed не должен маскировать известное нелицензированное копирование под пользовательскую загрузку.
...
- 2026-08-30: глобальный каталог Recording включён в STORY-008; вопрос пользовательских lyrics и notice-and-takedown вынесен в `EPIC-002-OQ-001` без принятия модели.
